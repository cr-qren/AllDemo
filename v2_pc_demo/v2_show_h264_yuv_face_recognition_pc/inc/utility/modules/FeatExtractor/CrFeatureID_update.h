...
class CrFeature
{
	...
	public:
		...
		// two parameters: input_face, face_feature
		...
		// 	{
		// 		int height = input_face[i].rows;
		// 		int width = input_face[i].cols;

		// 		copyMakeBorder(input_face[i], image_border, height, height, width, width, 0);

		// 		// imshow("source_image", input_face[i]);
		// 		// waitKey(0);
...
class CrFaceID
{
	private:
		...
		unordered_map<string, std::vector<matrix<float, 0, 1>>> idDatabase;
 		...
 		float dot, denom1, denom2, max_sim_ofdatabase, similarity, threshold;
 		// float maxsim_per_id2;

		// return distance beyond the threshold 
  ...
  //       std::vector<std::string> matchID(FaceSDK::CrFeature* ptr_feat, std::vector<cv::Mat>& input_img_array)
		// {
  //           if (input_img_array.ssize() <=0) return std::vector<std::string>();
  ...
  //           return belong_id;
		...
		// search id using euclidean distance
		// void matchID(unordered_map<string, std::vector<matrix<float, 0, 1>>>& idDatabase,std::vector<matrix<float,0,1>>& faces, std::vector<string>& belong_id)
		// {
