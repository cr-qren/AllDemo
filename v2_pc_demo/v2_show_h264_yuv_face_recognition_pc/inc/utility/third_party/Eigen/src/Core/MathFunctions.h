...
struct random_default_impl<Scalar, false, true> {
  static inline Scalar run(const Scalar& x, const Scalar& y) {
    typedef typename conditional<NumTraits<Scalar>::IsSigned, std::ptrdiff_t,
                                 std::size_t>::type ScalarX;
    if (y < x) return x;
    ...
    if (range < RAND_MAX)
      divisor = (std::size_t(RAND_MAX) + 1) / (range + 1);
    else
      multiplier = 1 + range / (std::size_t(RAND_MAX) + 1);
    do {
 ...
 *
 * It is essentially equivalent to \code using std::sqrt; return sqrt(x);
 * \endcode, but slightly faster for float/double and some compilers (e.g.,
 * gcc), thanks to specializations when SSE is enabled.
 *
